...
import { IERC20, IHarvestUsdcVault, IUniswapV2Pair, IcurveYSwap, HarvestAttack, Address } from '../typechain-types'
import { SignerWithAddress } from '@nomicfoundation/hardhat-ethers/signers'

describe('CCA Detects Harvest Attack Test', function () {
    let deployer: SignerWithAddress
    let harvestAttack: HarvestAttack

    let usdcPair: string
    let usdtPair: string
    let curveYSwap: string
    let harvest: string
    let usdc: string
    let usdt: string
    let fusdc: string
    ...
    beforeEach(async function () {
        ...
        usdcPair = await ethers.getAddress('0xB4e16d0168e52d35CaCD2c6185b44281Ec28C9Dc')
        usdtPair = await ethers.getAddress('0x0d4a11d5EEaaC28EC3F61d100daF4d40471f1852')
        curveYSwap = await ethers.getAddress('0x45F783CCE6B7FF23B2ab2D70e416cdb7D6055f51')
        harvest = await ethers.getAddress('0xf0358e8c3CD5Fa238a29301d0bEa3D63A17bEdBE')
        usdc = await ethers.getAddress('0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48')
        usdt = await ethers.getAddress('0xdAC17F958D2ee523a2206206994597C13D831ec7')
        fusdc = await ethers.getAddress('0xf0358e8c3CD5Fa238a29301d0bEa3D63A17bEdBE')
        fusdt = await ethers.getAddress('0x053c80eA73Dc6941F518a68E2FC52Ac45BDE7c9C')
        // 部署 HarvestAttack 合約
        const HarvestAttackFactory = await ethers.getContractFactory('HarvestAttack')
        harvestAttack = await HarvestAttackFactory.deploy(
            usdcPair,
            usdtPair,
            curveYSwap,
            harvest,
    ...
    it('should execute the attack using HarvestAttack contract', async function () {
        // Trigger the attack
        await harvestAttack.startAttack()
    })
})
